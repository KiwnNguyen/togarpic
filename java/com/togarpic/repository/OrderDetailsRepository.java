...
//			item.setPro_name(rs.getString("pro_name"));
	...
	public int insertODT(Orderdetails Orderdetails) {
		try {
				return db.update("exec orderODT ?,?,?",
			  new Object[] { Orderdetails.getOrd_id(),Orderdetails.getPro_id(),Orderdetails.getOdt_quantity()}); 
		}catch(Exception ec) {
			ec.printStackTrace();
