...
import com.togarpic.repository.OrderDetailsRepository;
import com.togarpic.repository.OrderRepository;
import com.togarpic.repository.ProductRepository;
import com.togarpic.repository.RecipeDetailsRepository;
...
public class AdminController implements WebMvcConfigurer {
		...
		String roles = (String) request.getSession().getAttribute("roles");
	        if (roles != null && roles.equals("ADMIN")) {
	 			try {
					...
					Iterable<com.togarpic.model.Category> listcate = cateRepo.findAll();
	...
	public String insertProduct(Model model) {
		try {
			List<com.togarpic.model.Category> cate = cateRepo.findAll();
	...
	public String showUpdateCategory(Model model, Category category, @PathVariable(name = "id") int id) {
		try {
			...
			com.togarpic.model.Category cat = cateRepo.findById(id);
...
//			cateRepo.update(category);
	...
	public String insertCategory(Model model, com.togarpic.model.Category category, @RequestParam("title") String title) {
		try {
			category.setCat_name(title);

			cateRepo.insert(category);
			Iterable<com.togarpic.model.Category> cat = cateRepo.findAll();
